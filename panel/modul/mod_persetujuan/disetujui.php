...
chk = (chk==true ? "Disetujui" : "");   var url = "p_persetujuan.php?userid="+uid+"&chkYesNo="+chk+"&iddaftar="+iddaftar; 
...
<h1>Data Yang Telah Disetujui</h1>
<div class="innerLR">	   
			...
			<table id="example" class="table-primary">
                ...
                <tbody>
					<?php
						...
						$querypersetujuan=mysql_query("SELECT * FROM penilaianmahasiswa 
						INNER JOIN pendaftaranmahasiswa ON pendaftaranmahasiswa.IDDAFTAR = penilaianmahasiswa.IDDAFTAR
						where pendaftaranmahasiswa.IDINFO=$idinfo AND STATUS_PERSETUJUAN=''") or die (mysql_error());
						$no=0;			
					...
					<input name="chk" type="checkbox"  <?php if($objectdata->STATUS_PERSETUJUAN=="Disetujui"){echo 'checked="checked"';}?> id="chk_<?php echo $objectdata->IDPENILAIAN; ?>" value="<?php echo $objectdata->IDPENILAIAN; ?>" onclick="chkit(<?php echo $objectdata->IDPENILAIAN; ?>,<?php echo $objectdata->IDDAFTAR; ?>, this.checked);" />Disetujui<br/>
					<?php
							echo'
								    </td>
